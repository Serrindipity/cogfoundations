...
```{r cover, echo = FALSE, out.width='100%', fig.align = 'center'}
 knitr::include_graphics("images/ch5/bike.jpg")
```
...
```{r serialposition, echo = FALSE, out.width='80%', fig.align = 'center', fig.cap = 'The serial position curve is the result of both primacy effects and recency effects.'}
# Data vectors
position <- 1:20
probability <- c(0.82, 0.77, 0.69, 0.60, 0.50, 0.45, 0.35, 0.45, 0.41, 0.39, 
                 0.38, 0.39, 0.45, 0.47, 0.50, 0.54, 0.62, 0.65, 0.85, 0.95)

# Create the plot
plot(position, probability, type = "n", 
     xlim = c(1, 20), ylim = c(0, 1.0),
     xlab = "Position in list", ylab = "Probability of recall",
     main = "", xaxt = "n")

# Add x-axis with tick marks at every increment
axis(1, at = 1:20, labels = FALSE)  # Add all tick marks without labels
axis(1, at = seq(2, 20, by = 2), labels = seq(2, 20, by = 2))  # Add labels every other increment

# Add background shading
rect(1, 0, 5, 1, col = "#C7EAFE", border = NA)  # Light blue (primacy)
rect(5, 0, 16, 1, col = "#F5F5F5", border = NA)  # Light gray (middle)
rect(16, 0, 20, 1, col = "#D3FFA5", border = NA)  # Light pink (recency)

# Add horizontal grid lines at increments of 0.2
abline(h = seq(0.2, 1.0, by = 0.2), col = "lightgray", lty = 1)

# Add the line and points
lines(position, probability, col = "#7A81FF", lwd = 2)
points(position, probability, col = "#7A81FF", pch = 19, cex = 0.8)

# Add axis labels
axis(1, at = c(2.5, 18), labels = c("Beginning", "End"), tick = FALSE, line = 1)
```
...
```{r structure, echo = FALSE, out.width='90%', fig.align = 'center', fig.cap = 'Structure of long-term memory.'}
knitr::include_graphics("images/ch5/fig2.png")
```
...
```{r library, echo = FALSE, out.width='60%', fig.align = 'center', fig.cap = 'Photo by Robert Bye on Uplash.'}
knitr::include_graphics("images/ch5/fig3.png")
```
...
```{r lop, echo = FALSE, out.width='35%', fig.align = 'center', fig.cap = 'Levels of processing based on evidence from Craik and Tulving.'}
knitr::include_graphics("images/ch5/fig4.png")
```
...
```{r albumbs, echo = FALSE, out.width='60%', fig.align = 'center', fig.cap = 'Photo albums can be effective sources of retrieval cues. Photo by BBH Singapore on Uplash.'}
knitr::include_graphics("images/ch5/book.png")
```
...
```{r morris, echo = FALSE, out.width='70%', fig.width=6, fig.height=5, fig.align = 'center', fig.cap = 'Results from Morris and colleagues’ Transfer-Appropriate Processing experiment [@Morris1977, Experiment 1].'}
# Data
recognition_data <- c(84.4, 63.3)  # Recognition test
rhyming_data <- c(33.3, 48.9)      # Rhyming test
recognition_se <- c(2.74, 4.22)    # Recognition SEs
rhyming_se <- c(3.96, 4.45)        # Rhyming SEs

# Create the plot with recognition test
plot(1:2, recognition_data, 
     type = "b", 
     pch = 16, 
     col = "#00B0F0", 
     lwd = 2,
     xlim = c(0.8, 2.2),
     ylim = c(0, 100),
     xlab = "Type of encoding task",
     ylab = "Percent recalled",
     xaxt = "n")

# Add rhyming test line
lines(1:2, rhyming_data, 
      type = "b", 
      pch = 1, 
      col = "#60BA01", 
      lwd = 2, 
...
# Add error bars
arrows(1:2, recognition_data - recognition_se, 1:2, recognition_data + recognition_se, 
       length = 0.1, angle = 90, code = 3, col = "#00B0F0")
arrows(1:2, rhyming_data - rhyming_se, 1:2, rhyming_data + rhyming_se, 
       length = 0.1, angle = 90, code = 3, col = "#60BA01")

# Add labels and legend
axis(1, at = 1:2, labels = c("Semantic encoding", "Rhyming encoding"))
legend("topright", 
       legend = c("Recognition test", "Rhyming test"),
       col = c("#00B0F0", "#60BA01"),
       pch = c(16, 1),
       lty = c(1, 2),
       lwd = 2)
